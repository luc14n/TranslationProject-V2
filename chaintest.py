...
class GeminiTranslationProvider(BaseTranslationProvider):
    def __init__(
        self, default_model: str = "gemini-2.5-pro"
    ):  # Fix 1: Standardized model ID
...
async def run_stochastic_chain(
    ...
    for i in range(len(family_chain) - 1):
        ...
        result = await provider.translate(
            text=current_text, source_lang=source_lang, target_lang=target_lang
        )
...
async def main():
    ...
    history = await run_stochastic_chain(
        test_text, chain_path, FAMILY_DICTIONARY, openai_provider
    )
