...
class PipelineVisualizer:
    ...
    def generate_radar_charts(self, dataset_name, provider_data):
        ...
        global_vals = [np.mean(all_l), np.mean(all_f), np.mean(all_b), np.mean(all_c), np.mean(all_t)]
        global_vals += global_vals[:1]

        for provider, scores in provider_data.items():
            if not scores['labse']: continue
            ...
            ax.plot(angles, global_vals, linewidth=1, linestyle='dashed', color=self.colors['GLOBAL_AVG'], label="Global Avg")
            ...
            plt.title(f"{provider}: {dataset_name}\n(Semantic Footprint vs Global)", size=14, y=1.1, fontweight='bold')
            plt.legend(loc='upper right', bbox_to_anchor=(1.2, 1.1))
            
            filename = os.path.join(self.output_dir, f"Radar_{dataset_name}_{provider}.png")
    ...
    def generate_judge_box_plot(self, all_judge_scores):
        ...
        sns.stripplot(x='Provider', y='Score', data=df, color=".3", size=5) # Add individual dots

        plt.title("Linguistic Integrity: Judge Score Distribution", fontsize=16, fontweight='bold')
    ...
    def generate_alignment_chart(self, dataset_name, provider_data):
        ...
        for provider in df['Provider'].unique():
            ...
            plt.plot(subset['Hop'], subset['COMET'], marker='o', label=provider, 
                     color=self.colors.get(provider, '#333333'), linewidth=2, markersize=7)

        plt.title(f"Semantic Trajectory: {dataset_name}", fontsize=16, fontweight='bold')
        plt.xlabel("Language Hop Number")
        plt.ylabel("COMET-QE (Translation Quality)")
        plt.xticks(range(1, 7), ["Hop 1\n(Germ)", "Hop 2\n(Rom)", "Hop 3\n(Slav)", "Hop 4\n(W.As)", "Hop 5\n(E.As)", "Hop 6\n(Eng)"])
